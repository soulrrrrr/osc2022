...
#define MMIO_BASE (0x3f000000) //I don't have virtual memory
...
int sd_mount() {
    ...
    // mount partition
    readblock(p1.starting_sector, buf); // 2048 (0x00000800)
    ...
    if (p1.partition_type == 0x0b) {  // FAT32 with CHS addressing
        ...
        getdir(mountpoint, &parent_dir, path_remain);
        ...
        // change to fat32
        mount_dir = mount_dir->mount->root;
        // fill internal data of mount node
        struct fat32_internal* root_internal = (struct fat32_internal*)malloc(sizeof(struct fat32_internal));
